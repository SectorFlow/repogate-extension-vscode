...
export class NpmWatcher {
    private watcher: vscode.FileSystemWatcher | undefined;
    private parser: NpmDependencyParser;
    private fileCache: Map<string, string> = new Map();
    private statusCache: Map<string, Map<string, DependencyInfo>> = new Map();
    private pollingIntervals: Map<string, NodeJS.Timeout> = new Map();
    private notifiedPackages: Map<string, Set<string>> = new Map(); // Track which notifications have been shown
    private hasRepository: boolean = false; // Whether workspace is linked to a Git repository
    ...
    constructor(
        ...
        this.parser = new NpmDependencyParser();
    }
    ...
    async start() {
        ...
        logger.info('NPM watcher started');
    }
    ...
    private async handleDependencyResponse(uri: vscode.Uri, dep: DependencyInfo, response: any) {
        ...
        // Initialize notification tracking for this file
        if (!this.notifiedPackages.has(uri.fsPath)) {
            this.notifiedPackages.set(uri.fsPath, new Set());
        }
        const notifiedSet = this.notifiedPackages.get(uri.fsPath)!;
        const notificationKey = `${dep.packageName}:${response.status}`;

        // Show notifications (only once per status)
    ...
    dispose() {
        ...
        logger.info('NPM watcher stopped');
    }
}
